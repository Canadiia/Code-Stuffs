...
try:
    ...
    with open(
        ...
        for lineIndex, line in enumerate(inputdata):
            ...
            if (
                ...
                if (
                    ...
                    lookahead = data[lineIndex : lineIndex + linesOfLookahead + 1]
        ...
        for tg in extractedTags[0] + extractedTags[2]:
            ...
            # search for any normal or hex unicode escape characters
            uni = re.findall("&#[^;x]*;", tg.content)
            unihex = re.findall("&#x[^;]*;", tg.content)

            # create an empty list to hold the original data and the converted character
            allchanges = []

            # if there's any normal unicode escape sequences
            if uni:

                # loop through them
                for sequence in uni:

                    # convert it into a character and add the original sequence and converted character to the array
                    character = chr(
                        int(sequence[sequence.find("#") + 1 : sequence.find(";")])
                    )
                    allchanges.append([sequence, character])

            # if there's any hex unicode escape sequences
            if unihex:

                # loop through them
                for sequence in unihex:

                    # convert it into a character and add the original sequence and converted character to the array
                    character = chr(
                        int(sequence[sequence.find("x") + 1 : sequence.find(";")], 16)
                    )
                    allchanges.append([sequence, character])
            ...
            for change in allchanges:
                ...
                tg.content = tg.content.replace(change[0], change[1])
